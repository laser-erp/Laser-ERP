...
| Задача | Статус |
|--------|--------|
| Доступ в Django-админку на проде | Сделано (суперпользователь `admin`, пароль менялся вручную в прошлом запуске — **не хранить в git**) |
...
| Сброс пароля **по email** (Яндекс) | **Не завершено**: SMTP host/user настроены, нужен `EMAIL_HOST_PASSWORD` = пароль приложения Яндекса |
| Код в репозитории | Ветка `cursor/ssh-and-password-reset-bbad`, PR: https://github.com/laser-erp/Laser-ERP/pull/1 |

---

## Секреты Cursor (обязательно проверить в начале)

Должны быть в **Secrets окружения Cloud Agent** (тот же список, что `VPS_PASSWORD`):
...
Если `YANDEX_SMTP_PASSWORD` **не set** — настройку SMTP на сервере не завершить; попросить пользователя добавить секрет и **перезапустить агента**.
...
# Записать VPS_SSH_PRIVATE_KEY в ~/.ssh/vps_key (нормализовать \\n → newline), chmod 600
...
EMAIL_HOST_PASSWORD=<ещё старый пароль Mail.ru — ЗАМЕНИТЬ>
```

### Шаги агента

1. Убедиться, что `YANDEX_SMTP_PASSWORD` есть в env агента.
2. Записать ключ SSH: `~/.ssh/vps_key` из `VPS_SSH_PRIVATE_KEY`.
...
Скрипт обновляет `/etc/laser-erp.env`, ставит `PASSWORD_RESET_EMAIL_ENABLED=1`, перезапускает `laser-erp`, шлёт тестовое письмо на `DEFAULT_FROM_EMAIL`.
...
### Суперпользователи и email (проверить)

```bash
sudo -E -u lasererp .venv/bin/python manage.py shell -c "
from django.contrib.auth import get_user_model
for u in get_user_model().objects.filter(is_superuser=True).order_by('username'):
    print(u.username, u.email)
"
```

У `admin` при необходимости сменить email в админке с `admin@example.com` на рабочий.
