...
Если `YANDEX_SMTP_PASSWORD` **не set** — SMTP на сервере не завершить. Попросить добавить секрет (пароль приложения Яндекса, без пробелов) и **продолжить/перезапустить агента**. Не печатать значение.
